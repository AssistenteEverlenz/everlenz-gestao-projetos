...
type ProfileState = { id: string; organization_id: string | null; full_name: string };
...
export async function getProfile() {
  ...
  const { data, error } = await supabase.from("profiles").select("id,organization_id,full_name").single();
  if (error) throw error;
  return data as ProfileState;
}

export async function createOrganization(name: string, slug: string) {
  const { data, error } = await getSupabaseBrowserClient().rpc("create_organization", { p_name: name, p_slug: slug });
  if (error) throw error;
...
export async function loadAvailableWorkspaces(userEmail: string) {
  ...
  if (!result.profile.organization_id) {
    const { data, error } = await getSupabaseBrowserClient().rpc("claim_project_invitations");
    if (error) throw error;
...
export async function loadWorkspaces(userEmail: string) {
  ...
  if (!profile.organization_id) return { profile, workspaces: [] as ProjectWorkspace[] };

  const { data: projectRows, error: projectError } = await supabase
    .from("projects")
    .select("id,organization_id,name,client_name,contract_number,description,address,start_date,planned_end_date,status")
    .order("created_at");
  ...
  const workspaces = await Promise.all((projectRows ?? []).map(async (row) => {
    ...
    const photoPaths = (feedRows ?? []).flatMap((item) => (item.photos ?? []).map((photo: { storage_path: string }) => photo.storage_path));
    const signedByPath = new Map<string, string>();
    if (photoPaths.length) {
      const { data: signed } = await supabase.storage.from("worksite-photos").createSignedUrls(photoPaths, 3600);
      signed?.forEach((item) => { if (item.path && item.signedUrl) signedByPath.set(item.path, item.signedUrl); });
    }
...
export async function inviteRemoteMember(projectId: string, member: Member) {
  const { error } = await getSupabaseBrowserClient().rpc("invite_project_member", {
...
export async function ensureRemoteStatusReport(projectId: string, reportDate: string) {
  const { data, error } = await getSupabaseBrowserClient().rpc("create_daily_status_report", {
    p_project_id: projectId,
...
export async function createRemoteProject(project: Project) {
  const { data, error } = await getSupabaseBrowserClient().rpc("create_project_with_owner", {
    p_name: project.name,
...
export async function createRemoteTask(projectId: string, task: Task, members: Member[], sortOrder: number) {
  const supabase = getSupabaseBrowserClient();
