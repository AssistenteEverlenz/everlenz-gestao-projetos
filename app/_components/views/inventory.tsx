...
import type { InventoryItem, InventoryRequest, Member, ProjectTeam, Task } from "../types";
...
export function Inventory({ items, tasks, members, projectTeams, saveItem, moveItem, deleteItem, createRequest, transitionRequest, importItems }: Props) {
...
function InventoryDetail({ item, tasks, initialTab, onClose, onMove, onEdit, onRequest, onTransition }: { item: InventoryItem; tasks: Task[]; initialTab: "movements" | "requests"; onClose: () => void; onMove: () => void; onEdit: () => void; onRequest: () => void; onTransition: (data: { request: InventoryRequest; status: RequestStatus }) => void }) {
  ...
  return <Modal title={item.name} subtitle={`${item.category}${item.sku ? ` · ${item.sku}` : ""}`} onClose={onClose} wide><div className="inventory-detail">
    ...
    {tab === "movements" ? <div className="inventory-history">{(item.movements ?? []).map((movement) => { const task = taskName(tasks, movement.taskId); return <article key={movement.id}><i className={movement.type}/><div><header><strong>{movementLabel[movement.type]} · {movement.quantity} {item.unit}</strong><span className="movement-reference"><b>{movement.internalCode}</b><time>{formatWhen(movement.createdAt)}</time></span></header><p>{movement.purpose}</p><footer><span>Registrado por <b>{movement.createdBy}</b></span>{movement.receiver && <span>Recebido por <b>{movement.receiver}</b></span>}{task && <span>EAP <b>{task.code} · {task.name}</b></span>}{movement.document && <span>Documento externo <b>{movement.document}</b></span>}<span>Saldo após: <b>{movement.balanceAfter} {item.unit}</b></span></footer></div></article>; })}{!item.movements?.length && <EmptyState text="Nenhuma movimentação registrada neste material."/>}</div>
...
function MovementForm({ item, tasks, receivers, processing, onClose, onSave }: { item: InventoryItem; tasks: Task[]; receivers: ReceiverOption[]; processing: boolean; onClose: () => void; onSave: (type: MovementType, quantity: number, taskId?: string, purpose?: string, receiver?: string, receiverKind?: ReceiverOption["kind"], receiverId?: string, document?: string) => Promise<void> }) {
  const [type, setType] = useState<MovementType>("entry"); const [quantity, setQuantity] = useState(1); const [taskId, setTaskId] = useState(""); const [purpose, setPurpose] = useState(""); const [receiverKey, setReceiverKey] = useState(""); const [document, setDocument] = useState("");
  const selectedReceiver = receivers.find((option) => `${option.kind}:${option.id}` === receiverKey);
  return <Modal title={`Movimentar · ${item.name}`} subtitle={`Saldo atual: ${item.quantity} ${item.unit}`} onClose={onClose}><form className="invite-form" onSubmit={(event) => { event.preventDefault(); void onSave(type, quantity, taskId || undefined, purpose, selectedReceiver?.name, selectedReceiver?.kind, selectedReceiver?.id, document || undefined); }}><label><span>Operação</span><select value={type} onChange={(event) => setType(event.target.value as MovementType)}><option value="entry">Entrada</option><option value="exit">Saída / retirada</option><option value="adjustment">Ajustar saldo para</option></select></label><label><span>{type === "adjustment" ? "Novo saldo" : "Quantidade"}</span><input required type="number" min={type === "adjustment" ? 0 : 0.001} max={type === "exit" ? item.quantity : undefined} step="0.001" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))}/></label>{type === "exit" && <label><span>Destinado à EAP</span><select value={taskId} onChange={(event) => setTaskId(event.target.value)}><option value="">Sem vínculo específico</option>{tasks.map((task) => <option value={task.id} key={task.id}>{task.code} · {task.name}</option>)}</select></label>}<label><span>Motivo / finalidade</span><textarea required rows={3} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="Compra, consumo na atividade, devolução, inventário..."/></label>{type === "exit" && <label><span>Quem recebeu</span><select required value={receiverKey} onChange={(event) => setReceiverKey(event.target.value)}><option value="">Selecione um usuário, equipe ou colaborador</option><optgroup label="Usuários da plataforma">{receivers.filter((option) => option.kind === "user").map((option) => <option key={`user:${option.id}`} value={`user:${option.id}`}>{option.label}</option>)}</optgroup><optgroup label="Equipes de campo">{receivers.filter((option) => option.kind === "team").map((option) => <option key={`team:${option.id}`} value={`team:${option.id}`}>{option.label}</option>)}</optgroup><optgroup label="Colaboradores das empresas">{receivers.filter((option) => option.kind === "worker").map((option) => <option key={`worker:${option.id}`} value={`worker:${option.id}`}>{option.label}</option>)}</optgroup></select>{!receivers.length && <small>Cadastre usuários ou colaboradores na tela Equipe antes de retirar.</small>}</label>}<div className="system-reference"><Icon name="lock"/><div><strong>Número interno automático</strong><p>O sistema criará uma referência única no formato MOV-000000 ao confirmar.</p></div></div><label><span>Documento externo (opcional)</span><input value={document} onChange={(event) => setDocument(event.target.value)} placeholder="Nota fiscal, OS ou protocolo externo"/></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>Cancelar</button><button className="primary-btn" disabled={processing || (type === "exit" && !selectedReceiver)}>{processing ? <><span className="button-spinner"/>Processando...</> : "Confirmar movimentação"}</button></div></form></Modal>;
}
