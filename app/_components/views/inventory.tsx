...
import type { InventoryItem, InventoryRequest, Task } from "../types";
...
type Props = {
  ...
  moveItem: (itemId: string, type: MovementType, quantity: number, taskId?: string, purpose?: string, receiver?: string, document?: string) => Promise<void>;
  ...
  transitionRequest: (requestId: string, status: RequestStatus, note?: string, receiver?: string, document?: string) => Promise<void>;
...
export function Inventory({ items, tasks, saveItem, moveItem, deleteItem, createRequest, transitionRequest, importItems }: Props) {
...
function InventoryDetail({ item, tasks, initialTab, onClose, onMove, onEdit, onRequest, onTransition }: { item: InventoryItem; tasks: Task[]; initialTab: "movements" | "requests"; onClose: () => void; onMove: () => void; onEdit: () => void; onRequest: () => void; onTransition: (data: { request: InventoryRequest; status: RequestStatus }) => void }) {
  ...
  return <Modal title={item.name} subtitle={`${item.category}${item.sku ? ` · ${item.sku}` : ""}`} onClose={onClose} wide><div className="inventory-detail">
    ...
    {tab === "movements" ? <div className="inventory-history">{(item.movements ?? []).map((movement) => { const task = taskName(tasks, movement.taskId); return <article key={movement.id}><i className={movement.type}/><div><header><strong>{movementLabel[movement.type]} · {movement.quantity} {item.unit}</strong><time>{formatWhen(movement.createdAt)}</time></header><p>{movement.purpose}</p><footer><span>Por <b>{movement.createdBy}</b></span>{movement.receiver && <span>Recebido por <b>{movement.receiver}</b></span>}{task && <span>EAP <b>{task.code} · {task.name}</b></span>}{movement.document && <span>Documento <b>{movement.document}</b></span>}<span>Saldo após: <b>{movement.balanceAfter} {item.unit}</b></span></footer></div></article>; })}{!item.movements?.length && <EmptyState text="Nenhuma movimentação registrada neste material."/>}</div>
...
function MovementForm({ item, tasks, processing, onClose, onSave }: { item: InventoryItem; tasks: Task[]; processing: boolean; onClose: () => void; onSave: (type: MovementType, quantity: number, taskId?: string, purpose?: string, receiver?: string, document?: string) => Promise<void> }) {
  const [type, setType] = useState<MovementType>("entry"); const [quantity, setQuantity] = useState(1); const [taskId, setTaskId] = useState(""); const [purpose, setPurpose] = useState(""); const [receiver, setReceiver] = useState(""); const [document, setDocument] = useState("");
  return <Modal title={`Movimentar · ${item.name}`} subtitle={`Saldo atual: ${item.quantity} ${item.unit}`} onClose={onClose}><form className="invite-form" onSubmit={(event) => { event.preventDefault(); void onSave(type, quantity, taskId || undefined, purpose, receiver || undefined, document || undefined); }}><label><span>Operação</span><select value={type} onChange={(event) => setType(event.target.value as MovementType)}><option value="entry">Entrada</option><option value="exit">Saída / retirada</option><option value="adjustment">Ajustar saldo para</option></select></label><label><span>{type === "adjustment" ? "Novo saldo" : "Quantidade"}</span><input required type="number" min={type === "adjustment" ? 0 : 0.001} max={type === "exit" ? item.quantity : undefined} step="0.001" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))}/></label>{type === "exit" && <label><span>Destinado à EAP</span><select value={taskId} onChange={(event) => setTaskId(event.target.value)}><option value="">Sem vínculo específico</option>{tasks.map((task) => <option value={task.id} key={task.id}>{task.code} · {task.name}</option>)}</select></label>}<label><span>Motivo / finalidade</span><textarea required rows={3} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="Compra, consumo na atividade, devolução, inventário..."/></label>{type === "exit" && <label><span>Quem recebeu</span><input required value={receiver} onChange={(event) => setReceiver(event.target.value)} placeholder="Pessoa ou equipe responsável"/></label>}<label><span>Documento / referência</span><input value={document} onChange={(event) => setDocument(event.target.value)} placeholder="NF, requisição, OS..."/></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>Cancelar</button><button className="primary-btn" disabled={processing}>{processing ? <><span className="button-spinner"/>Processando...</> : "Confirmar movimentação"}</button></div></form></Modal>;
}
...
function TransitionForm({ data, processing, onClose, onSave }: { data: { request: InventoryRequest; status: RequestStatus }; processing: boolean; onClose: () => void; onSave: (note?: string, receiver?: string, document?: string) => Promise<void> }) {
  const [note, setNote] = useState(""); const [receiver, setReceiver] = useState(""); const [document, setDocument] = useState(""); const fulfilling = data.status === "fulfilled";
  return <Modal title={fulfilling ? "Atender requisição" : data.status === "approved" ? "Aprovar requisição" : "Recusar requisição"} subtitle={`${data.request.quantity} unidades · ${data.request.purpose}`} onClose={onClose}><form className="invite-form" onSubmit={(event) => { event.preventDefault(); void onSave(note || undefined, receiver || undefined, document || undefined); }}>{fulfilling && <><label><span>Entregue para</span><input required value={receiver} onChange={(event) => setReceiver(event.target.value)} placeholder="Pessoa ou equipe que recebeu"/></label><label><span>Documento / referência</span><input value={document} onChange={(event) => setDocument(event.target.value)}/></label></>}<label><span>{data.status === "rejected" ? "Motivo da recusa" : "Observação"}</span><textarea required={data.status === "rejected"} rows={3} value={note} onChange={(event) => setNote(event.target.value)}/></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>Cancelar</button><button className={data.status === "rejected" ? "danger-btn" : "primary-btn"} disabled={processing}>{processing ? <><span className="button-spinner"/>Processando...</> : fulfilling ? "Atender e baixar estoque" : data.status === "approved" ? "Aprovar" : "Confirmar recusa"}</button></div></form></Modal>;
}
