  ...
  return <div className="view-stack inventory-view">
    ...
    {section === "items" && <section className="panel glass">
      ...
      <div className="inventory-filter-bar">
        ...
        <div className="inventory-filter-selects"><label><span>Categoria</span><SearchableSelect value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">Todas</option>{categories.map((category) => <option key={category}>{category}</option>)}</SearchableSelect></label><label><span>Atividade / EAP</span><SearchableSelect value={taskFilter} onChange={(event) => setTaskFilter(event.target.value)}><option value="">Todas</option>{tasks.map((task) => <option key={task.id} value={task.id}>{task.code} · {task.name}</option>)}</SearchableSelect></label>{(stockFilter !== "all" || categoryFilter || taskFilter) && <button className="text-btn" onClick={() => { setStockFilter("all"); setCategoryFilter(""); setTaskFilter(""); }}><Icon name="close"/> Limpar filtros</button>}</div>
      </div>
...
function MovementForm({ item, movement, tasks, receivers, processing, onClose, onSave }: { item: InventoryItem; movement?: InventoryMovement; tasks: Task[]; receivers: ReceiverOption[]; processing: boolean; onClose: () => void; onSave: (type: MovementType, quantity: number, taskId?: string, purpose?: string, receiver?: string, receiverKind?: ReceiverOption["kind"], receiverId?: string, document?: string) => Promise<void> }) {
  ...
  return <Modal title={movement ? `Editar · ${movement.internalCode}` : `Movimentar · ${item.name}`} subtitle={movement ? `${item.name} · os saldos posteriores serão recalculados` : `Saldo atual: ${item.quantity} ${item.unit}`} onClose={onClose}><form className="invite-form" onSubmit={(event) => { event.preventDefault(); void onSave(type, quantity, taskId || undefined, purpose, selectedReceiver?.name, selectedReceiver?.kind, selectedReceiver?.id, document || undefined); }}>{movement?.requestId && <div className="form-warning"><Icon name="info"/> Esta saída nasceu de uma requisição atendida. O tipo permanece como saída e as informações da requisição serão sincronizadas.</div>}<label><span>Operação</span><SearchableSelect value={type} disabled={!!movement?.requestId} onChange={(event) => setType(event.target.value as MovementType)}><option value="entry">Entrada</option><option value="exit">Saída / retirada</option><option value="adjustment">Ajustar saldo para</option></SearchableSelect></label><label><span>{type === "adjustment" ? "Novo saldo naquele momento" : "Quantidade"}</span><input required type="number" min={type === "adjustment" ? 0 : 0.001} max={!movement && type === "exit" ? item.quantity : undefined} step="0.001" value={quantity} onChange={(event) => setQuantity(Number(event.target.value))}/></label>{type === "exit" && <label><span>Destinado à EAP</span><SearchableSelect value={taskId} onChange={(event) => setTaskId(event.target.value)}><option value="">Sem vínculo específico</option>{tasks.map((task) => <option value={task.id} key={task.id}>{task.code} · {task.name}</option>)}</SearchableSelect></label>}<label><span>Motivo / finalidade</span><textarea required rows={3} value={purpose} onChange={(event) => setPurpose(event.target.value)} placeholder="Compra, consumo na atividade, devolução, inventário..."/></label>{type === "exit" && <label><span>Quem recebeu</span><SearchableSelect required value={receiverKey} onChange={(event) => setReceiverKey(event.target.value)}><option value="">Selecione um usuário, equipe ou colaborador</option><optgroup label="Usuários da plataforma">{receivers.filter((option) => option.kind === "user").map((option) => <option key={`user:${option.id}`} value={`user:${option.id}`}>{option.label}</option>)}</optgroup><optgroup label="Equipes de campo">{receivers.filter((option) => option.kind === "team").map((option) => <option key={`team:${option.id}`} value={`team:${option.id}`}>{option.label}</option>)}</optgroup><optgroup label="Colaboradores das empresas">{receivers.filter((option) => option.kind === "worker").map((option) => <option key={`worker:${option.id}`} value={`worker:${option.id}`}>{option.label}</option>)}</optgroup></SearchableSelect>{!receivers.length && <small>Cadastre usuários ou colaboradores na tela Equipe antes de retirar.</small>}</label>}<div className="system-reference"><Icon name={movement ? "info" : "lock"}/><div><strong>{movement ? `Referência preservada: ${movement.internalCode}` : "Número interno automático"}</strong><p>{movement ? "A alteração ficará registrada na trilha de auditoria." : "O sistema criará uma referência única no formato MOV-000000 ao confirmar."}</p></div></div><label><span>Documento externo (opcional)</span><input value={document} onChange={(event) => setDocument(event.target.value)} placeholder="Nota fiscal, OS ou protocolo externo"/></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={onClose}>Cancelar</button><button className="primary-btn" disabled={processing || (type === "exit" && !selectedReceiver)}>{processing ? <><span className="button-spinner"/>Processando...</> : movement ? "Salvar e recalcular" : "Confirmar movimentação"}</button></div></form></Modal>;
}
