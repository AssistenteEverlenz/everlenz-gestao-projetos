...
import type { JournalEntry, Member, Project, ProjectTeam, ProjectTeamMember, Task, ViewId } from "../types";
...
export function Team({
  ...
  async function submitEdit(event: React.FormEvent) {
    ...
    try {
      ...
      setEditing(null); setToast("Dados e permissões atualizados.");
  ...
  async function confirmDelete() {
    if (!deleting) return;
    setProcessing(true); setError("");
    try { await deleteMember(deleting); setDeleting(null); setToast("Acesso removido."); }
  ...
  async function submitOperationalTeam(event: React.FormEvent) {
    event.preventDefault(); setProcessing(true); setError("");
    try {
      await saveProjectTeam({ id: operationalTeam?.id ?? "", name: teamName, company: teamCompany, specialty: teamSpecialty, contact: teamContact, active: operationalTeam?.active ?? true, members: teamMembers.filter((member) => member.name.trim()).map((member) => ({ ...member, name: member.name.trim() })) });
  ...
  return <div className="view-stack team-view" data-tour="equipe-conteudo">
    <section className="team-summary glass">
      <div><div className="avatar-stack">{members.slice(0, 4).map((member) => <span className="avatar" style={{ background: member.color }} key={member.id}>{member.initials}</span>)}<span className="avatar rest">+{Math.max(0, members.length - 4)}</span></div><div><span className="overline">EQUIPE DO PROJETO</span><h2>{members.length} pessoas com acesso</h2><p>Administradores e gestores acessam todos os projetos da conta. Usuários acessam apenas os projetos em que forem incluídos.</p></div></div>
    ...
    <section className="permission-note"><Icon name="check"/><div><strong>Três perfis, regras objetivas</strong><p>Administrador tem controle total; Gestor coordena a operação; Usuário visualiza o Gantt e o Status Report e, no Diário de Obra, cria registros e edita ou exclui apenas os próprios.</p></div></section>

    {createOpen && <Modal title="Adicionar pessoa ao projeto" subtitle="Usuário: acesso a este projeto. Gestor ou administrador: acesso a todos os projetos da conta." onClose={() => !processing && setCreateOpen(false)}><form className="invite-form" onSubmit={submitCreate}><label><span>Nome completo</span><input value={name} required onChange={(event) => setName(event.target.value)}/></label><label><span>E-mail de acesso</span><input type="email" value={email} required onChange={(event) => setEmail(event.target.value)}/></label><label><span>Perfil de acesso</span><SearchableSelect value={role} onChange={(event) => setRole(event.target.value as Member["role"])}>{availableRoles.map((item) => <option key={item}>{item}</option>)}</SearchableSelect></label><div className="modal-note"><Icon name="check"/><p><strong>Primeiro acesso protegido</strong><br/>O usuário receberá uma senha provisória e será obrigado a criar uma nova senha ao entrar.</p></div>{error && <div className="access-message"><Icon name="alert"/>{error}</div>}<div className="modal-actions"><button type="button" className="secondary-btn" disabled={processing} onClick={() => setCreateOpen(false)}>Cancelar</button><button className="primary-btn" disabled={processing}>{processing && <span className="button-spinner"/>}{processing ? "Criando acesso..." : "Criar e liberar acesso"}</button></div></form></Modal>}
    {editing && <Modal title="Editar usuário" subtitle="As mudanças valem para todos os projetos da conta." onClose={() => !processing && setEditing(null)}><form className="invite-form" onSubmit={submitEdit}><label><span>Nome completo</span><input value={name} required onChange={(event) => setName(event.target.value)}/></label><label><span>E-mail de acesso</span><input value={editing.email} disabled/></label><label><span>Perfil de acesso</span><SearchableSelect value={role} disabled={editing.id === currentUserId && editing.role === "Administrador"} onChange={(event) => setRole(event.target.value as Member["role"])}>{availableRoles.includes(role) ? availableRoles.map((item) => <option key={item}>{item}</option>) : <option>{role}</option>}</SearchableSelect></label>{error && <div className="access-message"><Icon name="alert"/>{error}</div>}<div className="modal-actions"><button type="button" className="secondary-btn" disabled={processing} onClick={() => setEditing(null)}>Cancelar</button><button className="primary-btn" disabled={processing}>{processing && <span className="button-spinner"/>}{processing ? "Salvando..." : "Salvar alterações"}</button></div></form></Modal>}
    {deleting && <Modal title="Excluir usuário" subtitle={deleting.role === "Usuário" ? "Remove apenas o acesso a este projeto. O login e os demais projetos são preservados." : "Esta operação remove o login e o acesso a todos os projetos."} onClose={() => !processing && setDeleting(null)}><div className="invite-form"><div className="delete-warning"><Icon name="alert"/><p>Confirma a exclusão de <strong>{deleting.name}</strong>? Os registros feitos por essa pessoa permanecem na obra.</p></div>{error && <div className="access-message"><Icon name="alert"/>{error}</div>}<div className="modal-actions"><button className="secondary-btn" disabled={processing} onClick={() => setDeleting(null)}>Cancelar</button><button className="danger-btn" disabled={processing} onClick={confirmDelete}>{processing && <span className="button-spinner"/>}{processing ? "Excluindo..." : "Excluir usuário"}</button></div></div></Modal>}
    ...
    {operationalTeam !== undefined && <Modal title={operationalTeam ? "Editar equipe operacional" : "Nova equipe operacional"} subtitle="Agrupe os profissionais por empresa e especialidade." onClose={() => !processing && setOperationalTeam(undefined)} wide><form className="invite-form operational-team-form" onSubmit={submitOperationalTeam}><div className="form-grid"><label><span>Nome da equipe</span><input required value={teamName} onChange={(event) => setTeamName(event.target.value)} placeholder="Equipe elétrica A"/></label><label><span>Empresa</span><input required value={teamCompany} onChange={(event) => setTeamCompany(event.target.value)} placeholder="Empresa A"/></label><label><span>Especialidade</span><input required value={teamSpecialty} onChange={(event) => setTeamSpecialty(event.target.value)} placeholder="Elétrica, mecânica, civil..."/></label><label><span>Contato geral (opcional)</span><input value={teamContact} onChange={(event) => setTeamContact(event.target.value)} placeholder="Responsável ou telefone"/></label></div><section className="field-worker-editor"><header><div><span>COLABORADORES DA EMPRESA</span><p>Estas pessoas poderão ser selecionadas como destinatárias nas retiradas do estoque.</p></div><button type="button" className="secondary-btn compact" onClick={() => setTeamMembers((current) => [...current, { id: crypto.randomUUID(), name: "", role: "", phone: "", active: true }])}><Icon name="plus"/> Adicionar</button></header>{teamMembers.map((member, index) => <div className="field-worker-row" key={member.id}><label><span>Nome completo</span><input required value={member.name} onChange={(event) => setTeamMembers((current) => current.map((entry, position) => position === index ? { ...entry, name: event.target.value } : entry))}/></label><label><span>Função</span><input value={member.role ?? ""} onChange={(event) => setTeamMembers((current) => current.map((entry, position) => position === index ? { ...entry, role: event.target.value } : entry))} placeholder="Eletricista, encarregado..."/></label><label><span>Telefone</span><input value={member.phone ?? ""} onChange={(event) => setTeamMembers((current) => current.map((entry, position) => position === index ? { ...entry, phone: event.target.value } : entry))}/></label><button type="button" className="icon-btn tiny danger" aria-label={`Remover ${member.name || "colaborador"}`} onClick={() => setTeamMembers((current) => current.filter((_, position) => position !== index))}><Icon name="close"/></button></div>)}{!teamMembers.length && <div className="empty-inline"><Icon name="users"/><div><strong>Nenhum colaborador cadastrado</strong><p>Você ainda poderá selecionar a equipe inteira como destinatária.</p></div></div>}</section>{error && <div className="access-message"><Icon name="alert"/>{error}</div>}<div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setOperationalTeam(undefined)}>Cancelar</button><button className="primary-btn" disabled={processing}>{processing && <span className="button-spinner"/>}{processing ? "Salvando..." : "Salvar equipe e colaboradores"}</button></div></form></Modal>}
