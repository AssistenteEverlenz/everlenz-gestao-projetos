...
type Props = { project: Project; tasks: Task[]; entries: JournalEntry[]; members: Member[]; navigate: (view: ViewId) => void; metrics: { overall: number; active: number }; setMembers: Dispatch<SetStateAction<Member[]>>; setToast: (value: string) => void };

export function Team({ members, setMembers, setToast }: Props) {
  ...
  function submit(event: React.FormEvent) {
    ...
    setMembers((current) => [...current, { id: crypto.randomUUID(), name, email, role, initials, color: "#54756a", online: false }]);
  ...
  return <div className="view-stack team-view">
    ...
    {open && <Modal title="Convidar para a equipe" subtitle="A pessoa receberá acesso ao projeto atual." onClose={() => setOpen(false)}><form className="invite-form" onSubmit={submit}><label><span>Nome completo</span><input value={name} required onChange={(event) => setName(event.target.value)} placeholder="Nome da pessoa"/></label><label><span>E-mail</span><input type="email" value={email} required onChange={(event) => setEmail(event.target.value)} placeholder="nome@empresa.com.br"/></label><label><span>Função no projeto</span><select value={role} onChange={(event) => setRole(event.target.value as Member["role"])}><option>Engenheiro</option><option>Encarregado</option><option>Cliente</option><option>Administrador</option></select></label><div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setOpen(false)}>Cancelar</button><button className="primary-btn">Enviar convite</button></div></form></Modal>}
  </div>;
}
