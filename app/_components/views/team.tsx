...
export function Team({ members, inviteMember, setToast }: Props) {
  ...
  return <div className="view-stack team-view">
    ...
    {open && <Modal title="Novo usuário" subtitle="O login será criado imediatamente, sem confirmação por e-mail." onClose={() => setOpen(false)}><form className="invite-form" onSubmit={submit}><label><span>Nome completo</span><input value={name} required onChange={(event) => setName(event.target.value)} placeholder="Nome da pessoa"/></label><label><span>E-mail de acesso</span><input type="email" value={email} required onChange={(event) => setEmail(event.target.value)} placeholder="nome@empresa.com.br"/></label><label><span>Função no projeto</span><select value={role} onChange={(event) => setRole(event.target.value as Member["role"])}><option>Gestor</option><option>Engenheiro</option><option>Encarregado</option><option>Cliente</option><option>Administrador</option></select></label><div className="modal-note"><Icon name="check"/><p><strong>Acesso imediato</strong><br/>Ao salvar, o Supabase cria o usuário com e-mail confirmado e gera uma senha provisória para você repassar.</p></div>{error && <div className="access-message"><Icon name="alert"/>{error}</div>}<div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setOpen(false)}>Cancelar</button><button className="primary-btn" disabled={saving}>{saving ? "Criando acesso..." : "Criar usuário"}</button></div></form></Modal>}
