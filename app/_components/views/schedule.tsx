...
type Props = { project: Project; tasks: Task[]; entries: JournalEntry[]; members: Member[]; navigate: (view: ViewId) => void; metrics: { overall: number; active: number }; addTask: (task: Task) => void; updateTaskProgress: (id: string, progress: number) => void; setToast: (value: string) => void };
...
function TaskForm({ project, tasks, members, onClose, onSave }: { project: Project; tasks: Task[]; members: Member[]; onClose: () => void; onSave: (task: Task) => void }) {
  ...
  const [name, setName] = useState(""); const [code, setCode] = useState(String(tasks.length + 1)); const [phase, setPhase] = useState("");
