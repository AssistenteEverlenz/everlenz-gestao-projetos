...
function JournalForm({ tasks, date, onClose, onSave }: { tasks: Task[]; date: string; onClose: () => void; onSave: (entry: JournalEntry) => void }) {
  ...
  function changeTask(value: number) { setTaskId(value); const next = tasks.find((item) => item.id === value)!; setProgress(Math.min(5, 100 - next.progress)); }
  ...
  return <Modal title="Novo registro de campo" subtitle="As fotos e a medição ficarão vinculadas à atividade selecionada." onClose={onClose} wide><form className="journal-form field-first" onSubmit={submit}>
    <label className="full activity-selector"><span>1 · Atividade do cronograma</span><select value={taskId} onChange={(event) => changeTask(Number(event.target.value))}>{activeTasks.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name} ({item.progress}%)</option>)}</select><small>{task.responsible || "Sem responsável"} · {task.phase}</small></label>
