...
export function AuthScreen() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (mode === "signup") {
      ...
      setMessage(error ? translateAuthError(error.message) : data.session ? "Conta criada. Preparando seu ambiente..." : "Conta criada. Você já pode entrar com seu e-mail e senha.");
    } else {
...
function translateAuthError(message: string) {
  ...
  if (normalized.includes("invalid login credentials")) return "Não encontramos uma conta com este e-mail ou a senha está incorreta.";
  if (normalized.includes("email not confirmed")) return "Este acesso ainda está aguardando liberação. Fale com o administrador.";
  if (normalized.includes("user already registered")) return "Já existe uma conta com este e-mail. Use a opção de entrar.";
