...
export function EntryHistoryModal({
  ...
  return (
    <Modal
      title={task.name}
      subtitle={`${task.code} · ${ordered.length} registro${ordered.length === 1 ? "" : "s"} no Diário de Obra`}
      onClose={onClose}
      wide
    >
      {editing ? (
        ...
        <div className="entry-history">
          {!ordered.length && (
            <div className="entry-history-empty">
              <Icon name="journal" />
              <strong>Nenhum diário vinculado</strong>
              <span>
                Os registros de campo desta atividade aparecerão aqui.
              </span>
            </div>
          )}
          {ordered.map((entry) => (
            <article className="entry-history-item" key={entry.id}>
              <header>
                <div>
                  <span>
                    {fullDate(entry.date)} · {entry.time}
                  </span>
                  <h3>{entry.title}</h3>
                  <small>
                    {entry.author} · {entry.weather} · {entry.crew} pessoa
                    {entry.crew === 1 ? "" : "s"}
                  </small>
                </div>
                <div className="entry-history-actions">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (
                        !window.confirm(
                          `Excluir definitivamente o diário “${entry.title}”? O avanço será recalculado.`,
                        )
                      )
                        return;
              ...
              <div className="history-progress">
                <span>
                  <small>ANTES</small>
                  <b>{entry.progressBefore}%</b>
                </span>
                <Icon name="arrow" />
                <span className="daily">
                  <small>MEDIDO</small>
                  <b>+{entry.progressAdded}%</b>
                </span>
                <Icon name="arrow" />
                <span>
                  <small>DEPOIS</small>
                  <b>{entry.progressAfter}%</b>
                </span>
              </div>
                  ...
                  {entry.photos.map((photo, index) => (
                    <button
                      ...
                      onClick={() =>
                        setPreviewPhoto({
                          url: photo.url,
                          label: `${entry.title} · Foto ${index + 1}`,
                        })
                      }
                    >
                      <img
      ...
      {previewPhoto && (
        ...
        >
          ...
          <img src={previewPhoto.url} alt={previewPhoto.label} />
        </div>
      )}
    </Modal>
  );
...
function EntryEditForm({
  ...
  async function addPhotos(files: FileList | null) {
    ...
    try {
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Não foi possível processar as fotos.",
      );
    } finally {
      setProcessing(false);
    }
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      await onSave({
        ...
        progressAfter: entry.progressBefore + progress,
        crew,
        weather,
        photos,
      });
  ...
  return (
    <form className="entry-edit-form" onSubmit={submit}>
      <div className="edit-context">
        <Icon name="journal" />
        <span>
          <strong>Editando registro de {fullDate(entry.date)}</strong>
          <small>
            A atividade e a data permanecem vinculadas para preservar a
            rastreabilidade.
          </small>
        </span>
      </div>
      ...
      <div className="entry-edit-grid">
        <label>
          ...
          />
          <small>
            Avanço atual da atividade após a correção:{" "}
            {progressWithoutEntry + progress}%
          </small>
        </label>
        <label>
          <span>Efetivo</span>
      ...
      <label className="photo-drop compact-photo-drop">
        ...
        <small>
          As novas imagens serão redimensionadas e comprimidas antes do envio.
        </small>
        <em>{processing ? "Compactando..." : `${photos.length}/8 fotos`}</em>
      </label>
      {photos.length > 0 && (
        <div className="upload-grid">
          {photos.map((photo, index) => (
            <div key={photo.id ?? `${photo.url.slice(0, 24)}-${index}`}>
      ...
      <div className="modal-actions">
        ...
        <button className="primary-btn" disabled={saving || processing}>
          <Icon name="check" />
